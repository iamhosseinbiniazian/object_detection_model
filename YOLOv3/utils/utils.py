...
def evaluate(model, 
    ...
    for i in tqdm(range(generator.size())):
        ...
        score = np.array([box.get_score() for box in pred_boxes[0]])
        pred_labels = np.array([box.label for box in pred_boxes[0]])
...
def get_yolo_boxes(model, images, net_h, net_w, anchors, obj_thresh, nms_thresh):
    image_h, image_w = images.shape[0],images.shape[1]
    nb_images           = 1
    batch_input         = np.zeros((nb_images, net_h, net_w, 3))

    # preprocess the input
    for i in range(nb_images):
        batch_input[i] = preprocess_input(images, net_h, net_w)
