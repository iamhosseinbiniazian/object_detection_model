...
def create_model(nb_class, anchors, max_box_per_image, max_grid, batch_size, warmup_batches, ignore_thresh, multi_gpu,
                 saved_weights_name):
    if multi_gpu > 1:
    ...
    if os.path.exists(saved_weights_name):
        ...
        print('Load weights {}.'.format('backend.h5'))
        freeze_body = 2
        if freeze_body in [1, 2]:
            # Freeze darknet53 body or freeze all but 3 output layers.
            num = (185, len(template_model.layers) - 3)[freeze_body - 1]
            for i in range(num): template_model.layers[i].trainable = False
            print('Freeze the first {} layers of total {} layers.'.format(num, len(template_model.layers)))
