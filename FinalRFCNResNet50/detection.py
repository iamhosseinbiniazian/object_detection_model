...
class AUTDetection(VisionDataset):
    ...
    """
    CLASSES = ('background','car','cementsilo','cementtruck','loader','person','tank')

    def __init__(self, root=os.path.join('~', 'workspace', 'Data'),
                 splits=((2007, 'trainval')),
                 transform=None, index_map=None, preload_label=True):
        super(AUTDetection, self).__init__(root)
        self._im_shapes = {}
    ...
    def _load_items(self, splits):
        ...
        ids = []
        for name in splits:
            root = os.path.join(self._root,str(name))
